...
const createTxWrapper = (prismaClient) => ({
  ...
  createMany: ({ model, data, skipDuplicates }) =>
...
export const createMany = ({ model, data, skipDuplicates }) => {
...
export const max = ({ model, where = {} }) => {
  return getClient(model).max({ where });
};


export const groupBy = ({ model, by, where = {}, _count, _sum, _avg, _min, _max, orderBy, having }) => {
